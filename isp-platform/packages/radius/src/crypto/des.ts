/**
 * Pure-JS single-block DES-ECB encryption (FIPS 46-3). Needed for MS-CHAPv2's
 * ChallengeResponse step; Node's OpenSSL 3 default provider no longer exposes
 * legacy DES ciphers. Only single 8-byte block encryption (no padding, no
 * chaining) is implemented, since that's all MS-CHAPv2 requires.
 */
...
function bytesToBits(buf: Buffer): Bit[] {
  const bits: Bit[] = [];
...
function bitsToBytes(bits: Bit[]): Buffer {
...
/** 1-indexed permutation table lookup, as specified by FIPS 46-3. */
...
function generateSubkeys(key: Buffer): Bit[][] {
...
function feistel(r: Bit[], subkey: Bit[]): Bit[] {
  const expanded = permute(r, E);
  const x = xorBits(expanded, subkey);

  const sboxOutput: Bit[] = [];
  for (let i = 0; i < 8; i++) {
    const block = x.slice(i * 6, i * 6 + 6);
    const row = (block[0] << 1) | block[5];
    const col = (block[1] << 3) | (block[2] << 2) | (block[3] << 1) | block[4];
    const value = S_BOXES[i][row * 16 + col];
    for (let b = 3; b >= 0; b--) {
      sboxOutput.push(((value >> b) & 1) as Bit);
    }
  }
...
/** Encrypts exactly one 8-byte block with an 8-byte DES key (parity bits ignored). */
export function desEncryptBlock(key: Buffer, block: Buffer): Buffer {
  if (key.length !== 8 || block.length !== 8) {
    throw new RangeError("desEncryptBlock requires 8-byte key and 8-byte block.");
  }
  ...
  for (let round = 0; round < 16; round++) {
    const nextL = r;
    const nextR = xorBits(l, feistel(r, subkeys[round]));
    l = nextL;
    r = nextR;
  }
